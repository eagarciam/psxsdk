PSXSDK 0.6.2 Additional Notes
Section last updated: September 22nd 2019
-----------------------------

This git repository is a mirror of the open source PSXSDK found on 
http://unhaut.x10host.com/psxsdk/. 
The version of this repository is 20190410 (0.6.2).

A precompiled toolchain and SDK (with C++ support) for Linux operating systems 
is included in the Releases section of this repository as links for precompiled 
toolchains on the main site homepage are currently down.

The provided toolchain and SDK have been built with GCC 5.4.0 on Ubuntu 16.04 
LTS. The MIPS toolchain itself was compiled from the GCC 8.3.0 and GNU Binutils 
2.32 sources. Lastly, the SDK was compiled from an unmodified PSXSDK 0.6.2 
source acquired on the aforementioned website.

The MIPS toolchain and PSXSDK together provide all the tools necessary for 
developing software for the PlayStation 1. The MIPS toolchain provides a cross-
compiler and linker targeting the MIPS instruction set used by the PlayStation 1
while the SDK provides an API/library for interfacing with system functionality 
and various tools such as PS1-specific file format converters or CD image 
generators. Read the section below containing the original readme.txt text to 
get an idea of how it works. You may also wish to check out the sample code 
located in the examples folder in this repository to see how to interface with 
PSXSDK. As mentioned in the original readme text below, it's also a good chance 
to test if the toolchain (whether acquired from Releases or built yourself) 
works properly. Unfortunately this documentation won't really teach you in 
detail how to program for the PS1. Familiarity with programming on bare metal 
hardware or modern embedded devices is recommended.

The only files that have been modified in this repository are this readme.txt 
...
least for what I have seen for images with 2352 byte sectors). This means that 
not all emulators, such as Mednafen 0.8.4+, will be able to run the game without
running your image through an error correcting program. I am not sure if the 
incorrect EDC/L-EDC data is due to genisoimage/mkisofs or due to the PSXSDK's 
mkpsxiso utility. The workaround is to either just play the .exe outputted from 
elf2exe or to run your .bin CD image through an EDC/L-EDC corrector so that it 
can be played properly in Mednafen.

Some emulators will throw a video mismatch error if the video mode (NTSC or PAL)
does not match the region specified by the license file the iso is created with.
Just make sure this matches, unless you're making a program that can choose or 
switch between NTSC and PAL. In that case you'll have to find an emulator that 
isn't too picky on video mode or if all else fails, just run on original 
hardware.

PSXSDK-built games don't seem to run at full speed or will otherwise manifest
various graphical glitches in a number of emulators. Whether this is due to 
many emulators being strict on how they handle certain operations or due to 
emulation inaccuracy is something I am unsure of. However, programs compiled 
with the official PSY-Q SDK seem to run at full speed on emulators without 
issues from my experience.

Additional Notes written by ggdrt
